...
module.exports = ({logger, sport}) => (data) => pgConnect(logger, (client, done) => {
  ...
  async.series([
    ...
    async.parallel([
      (cb) => client.query(`NOTIFY entries, '${sport}-${year}';`, cb),
      (cb) => client.query(`NOTIFY users, '${data.user_id}';`, cb)
    ], (notifyErr) => {
      done(notifyErr);
      logger.debug(`notify entries, users: ${notifyErr || 'Success'}`);
    });
  });
});
